%input data
%%system data
sysFile='D:\GitHub\cross-region_accomodation\data\2area_12bus\sys.xlsx';
A=xlsread(sysFile,'B1:B1');               %% number of areas
T=xlsread(sysFile,'B2:B2');               %% time horizons
TD=0;                                     %% number of days

areaFile{1}='D:\GitHub\cross-region_accomodation\data\2area_12bus\area_1.xlsx';
areaFile{2}='D:\GitHub\cross-region_accomodation\data\2area_12bus\area_2.xlsx';
Nunit = [3,2];            %% number of units
...
for a=1:A
    ...
    %%----------------------------- tie-line data -----------------------------
    Tieline{a}     = xlsread(areaFile{a},4,['B2:D' num2str(Ntie(a)+1)]);
